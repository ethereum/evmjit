...
Compiler::Compiler(
    Options const& _options, evmc_revision _rev, bool _staticCall, llvm::LLVMContext& _llvmContext)
  : m_options(_options), m_rev(_rev), m_staticCall(_staticCall), m_builder(_llvmContext)
{
    Type::init(m_builder.getContext());
}

std::vector<BasicBlock> Compiler::createBasicBlocks(
...
void Compiler::resolveJumps()
{
    ...
    for (auto it = std::next(m_mainFunc->begin()), end = std::prev(m_mainFunc->end(), 4); it != end;
         ++it)
    {
        ...
        {
            auto destIdx =
                llvm::cast<llvm::ValueAsMetadata>(jump->getMetadata(c_destIdxLabel)->getOperand(0))
                    ->getValue();
...
std::unique_ptr<llvm::Module> Compiler::compile(
    code_iterator _begin, code_iterator _end, std::string const& _id)
{
    auto module = llvm::make_unique<llvm::Module>(
        _id, m_builder.getContext());  // TODO: Provide native DataLayout

    // Create main function
    auto mainFuncType = llvm::FunctionType::get(Type::MainReturn, Type::RuntimePtr, false);
    m_mainFunc =
        llvm::Function::Create(mainFuncType, llvm::Function::ExternalLinkage, _id, module.get());
    ...
    // Special "Stop" block. Guarantees that there exists a next block after the code blocks (also
    // when there are no code blocks).
    ...
    auto frameaddress =
        llvm::Intrinsic::getDeclaration(module.get(), llvm::Intrinsic::frameaddress);
    ...
    Arith256& _arith, Memory& _memory, Ext& _ext, GasMeter& _gasMeter)
{
    ...
    for (auto it = _basicBlock.begin(); it != _basicBlock.end(); ++it)
    {
        ...
        case Instruction::DIV:
        {
            ...
            n = m_builder.CreateSelect(divByZero, Constant::get(1), n);  // protect against hardware
                                                                         // signal
        ...
        case Instruction::SDIV:
        {
            ...
            r = m_builder.CreateSelect(divByMinusOne, dNeg, r);  // protect against undef i256.min /
                                                                 // -1
            stack.push(r);
            break;
        }
        ...
        case Instruction::SMOD:
        {
            ...
            r = m_builder.CreateSelect(divByZero, Constant::get(0), r);
            r = m_builder.CreateSelect(divByMinusOne, Constant::get(0), r);  // protect against
                                                                             // undef i256.min / -1
            stack.push(r);
            break;
        }
        ...
        case Instruction::BYTE:
        {
            ...
            auto bytes =
                m_builder.CreateBitCast(value, llvm::VectorType::get(Type::Byte, 32), "bytes");
        ...
        case Instruction::SIGNEXTEND:
        {
            ...
            auto result =
                m_builder.CreateSelect(kInRange, m_builder.CreateSelect(bittest, val1, val0), word);
            stack.push(result);
            break;
        }
        ...
        {
            auto destIdx =
                llvm::MDNode::get(m_builder.getContext(), llvm::ValueAsMetadata::get(stack.pop()));

            // Create branch instruction, initially to jump table.
            // Destination will be optimized with direct jump during jump resolving if destination
            // index is a constant.
            auto jumpInst = (inst == Instruction::JUMP) ?
                                m_builder.CreateBr(m_jumpTableBB) :
                                m_builder.CreateCondBr(m_builder.CreateICmpNE(stack.pop(),
                                                           Constant::get(0), "jump.check"),
                                    m_jumpTableBB, nullptr);
        ...
        case Instruction::ORIGIN:
            stack.push(m_builder.CreateZExt(
                Endianness::toNative(m_builder, _runtimeManager.getTxContextItem(1)), Type::Word));
            break;
        case Instruction::COINBASE:
            stack.push(m_builder.CreateZExt(
                Endianness::toNative(m_builder, _runtimeManager.getTxContextItem(2)), Type::Word));
            break;
        ...
        case Instruction::CODECOPY:
        {
            ...
            auto srcPtr = _runtimeManager.getCode();  // TODO: Code & its size are constants,
                                                      // feature #80814234
        ...
        case Instruction::CREATE:
        {
            ...
            auto rmagic = m_builder.CreateSelect(
                ret, m_builder.getInt64(0), m_builder.getInt64(EVM_CALL_FAILURE), "call.rmagic");
            // TODO: optimize
        ...
        case Instruction::STATICCALL:
        {
            ...
            auto transferCost = m_builder.CreateSelect(
                noTransfer, m_builder.getInt64(0), m_builder.getInt64(transferGas));
            ...
            if (inst == Instruction::CALL)
            {
                ...
                auto penalty = m_builder.CreateSelect(noPenaltyCond, m_builder.getInt64(0),
                    m_builder.getInt64(JITSchedule::callNewAccount::value));
                _gasMeter.count(penalty, _runtimeManager.getJmpBuf(), _runtimeManager.getGasPtr());
            }
            ...
            if (m_rev >= EVMC_TANGERINE_WHISTLE)
            {
                ...
                auto gasMaxAllowed = m_builder.CreateZExt(
                    m_builder.CreateSub(gas, gas64th, "gas.maxallowed", true, true), Type::Word);
            ...
            auto stipend = m_builder.CreateSelect(noTransfer, m_builder.getInt64(0),
        ...
        case Instruction::SUICIDE:
        {
            ...
            if (m_rev >= EVMC_TANGERINE_WHISTLE)
            {
                ...
                if (m_rev >= EVMC_SPURIOUS_DRAGON)
                {
                    ...
                    auto noTransfer = m_builder.CreateICmpEQ(balance, Constant::get(0));
