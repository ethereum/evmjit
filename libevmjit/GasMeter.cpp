...
GasMeter::GasMeter(IRBuilder& _builder, RuntimeManager& _runtimeManager, evmc_revision rev)
  : CompilerHelper(_builder), m_runtimeManager(_runtimeManager), m_rev(rev)
{
    llvm::Type* gasCheckArgs[] = {Type::Gas->getPointerTo(), Type::Gas, Type::BytePtr};
    m_gasCheckFunc =
        llvm::Function::Create(llvm::FunctionType::get(Type::Void, gasCheckArgs, false),
            llvm::Function::PrivateLinkage, "gas.check", getModule());
    ...
    auto gasOk = m_builder.CreateICmpSGE(gasUpdated, m_builder.getInt64(0),
        "gasOk");  // gas >= 0, with gas == 0 we can still do 0 cost instructions
...
void GasMeter::count(Instruction _inst)
{
    if (!m_checkCall)
    {
        ...
        m_checkCall = m_builder.CreateCall(
            m_gasCheckFunc, {m_runtimeManager.getGasPtr(), llvm::UndefValue::get(Type::Gas),
                                m_runtimeManager.getJmpBuf()});
    }
...
void GasMeter::count(llvm::Value* _cost, llvm::Value* _jmpBuf, llvm::Value* _gasPtr)
{
    ...
    m_builder.CreateCall(m_gasCheckFunc, {_gasPtr ? _gasPtr : m_runtimeManager.getGasPtr(), _cost,
                                             _jmpBuf ? _jmpBuf : m_runtimeManager.getJmpBuf()});
}
...
void GasMeter::countExp(llvm::Value* _exponent)
{
    ...
    auto sigBytes = m_builder.CreateUDiv(
        m_builder.CreateAdd(sigBits, m_builder.getInt64(7)), m_builder.getInt64(8));
...
void GasMeter::countSStore(Ext& _ext, llvm::Value* _index, llvm::Value* _newValue)
{
    ...
    assert(JITSchedule::sstoreResetGas::value == JITSchedule::sstoreClearGas::value &&
           "Update SSTORE gas cost");
    auto cost =
        m_builder.CreateSelect(isInsert, m_builder.getInt64(JITSchedule::sstoreSetGas::value),
            m_builder.getInt64(JITSchedule::sstoreResetGas::value), "cost");
    count(cost);
}
...
void GasMeter::countLogData(llvm::Value* _dataLength)
{
    ...
    assert(
        JITSchedule::logDataGas::value != 1 && "Log data gas cost has changed. Update GasMeter.");
    count(m_builder.CreateNUWMul(
        _dataLength, Constant::get(JITSchedule::logDataGas::value)));  // TODO: Use i64
}
...
void GasMeter::countSha3Data(llvm::Value* _dataLength)
{
    ...
    auto words64 = m_builder.CreateUDiv(
        m_builder.CreateNUWAdd(dataLength64, m_builder.getInt64(31)), m_builder.getInt64(32));
    auto cost64 =
        m_builder.CreateNUWMul(m_builder.getInt64(JITSchedule::sha3WordGas::value), words64);
    count(cost64);
}
...
void GasMeter::commitCostBlock()
{
    ...
    if (m_checkCall)
    {
        ...
        m_checkCall->setArgOperand(1, m_builder.getInt64(m_blockCost));  // Update block cost in gas
                                                                         // check call
