...
llvm::Twine getName(RuntimeData::Index _index)
{
    ...
    default:
        return "";
    case RuntimeData::Gas:
        return "msg.gas";
    case RuntimeData::GasPrice:
        return "tx.gasprice";
    case RuntimeData::CallData:
        return "msg.data.ptr";
    case RuntimeData::CallDataSize:
        return "msg.data.size";
    case RuntimeData::Value:
        return "msg.value";
    case RuntimeData::Code:
        return "code.ptr";
    case RuntimeData::CodeSize:
        return "code.size";
    case RuntimeData::Address:
        return "msg.address";
    case RuntimeData::Sender:
        return "msg.sender";
    case RuntimeData::Depth:
        return "msg.depth";
    }
}
}  // namespace
...
RuntimeManager::RuntimeManager(
    IRBuilder& _builder, code_iterator _codeBegin, code_iterator _codeEnd)
  : CompilerHelper(_builder), m_codeBegin(_codeBegin), m_codeEnd(_codeEnd)
{
    ...
    auto getTxCtxFnTy =
        llvm::FunctionType::get(Type::Void, {m_txCtx->getType(), Type::EnvPtr}, false);
    auto getTxCtxFn = llvm::Function::Create(
        getTxCtxFnTy, llvm::Function::ExternalLinkage, "evm.get_tx_context", getModule());
    auto loadTxCtxFnTy = llvm::FunctionType::get(
        Type::Void, {m_txCtxLoaded->getType(), m_txCtx->getType(), Type::EnvPtr}, false);
    m_loadTxCtxFn = llvm::Function::Create(
        loadTxCtxFnTy, llvm::Function::PrivateLinkage, "loadTxCtx", getModule());
    ...
    m_dataPtr =
        m_builder.CreateLoad(m_builder.CreateStructGEP(getRuntimeType(), rtPtr, 0), "dataPtr");
    ...
    auto mallocFunc =
        llvm::Function::Create(llvm::FunctionType::get(Type::WordPtr, {Type::Size}, false),
            llvm::Function::ExternalLinkage, "malloc", getModule());
    mallocFunc->setDoesNotThrow();
    mallocFunc->addAttribute(0, llvm::Attribute::NoAlias);

    m_stackBase = m_builder.CreateCall(mallocFunc,
        m_builder.getInt64(Type::Word->getPrimitiveSizeInBits() / 8 * stackSizeLimit),
        "stack.base");  // TODO: Use Type::SizeT type
    ...
    if (!freeFunc)
    {
        freeFunc = llvm::Function::Create(llvm::FunctionType::get(Type::Void, Type::WordPtr, false),
            llvm::Function::ExternalLinkage, "free", getModule());
        freeFunc->setDoesNotThrow();
        freeFunc->addAttribute(1, llvm::Attribute::NoCapture);
    }
    m_builder.CreateCall(freeFunc, {m_stackBase});
    auto extGasPtr = m_builder.CreateStructGEP(
        getRuntimeDataType(), getDataPtr(), RuntimeData::Index::Gas, "msg.gas.ptr");
...
llvm::Value* RuntimeManager::getDataPtr()
{
    ...
    auto rtPtr = getRuntimePtr();
    auto dataPtr =
        m_builder.CreateLoad(m_builder.CreateStructGEP(getRuntimeType(), rtPtr, 0), "data");
...
llvm::Value* RuntimeManager::getCode()
{
    ...
    if (!m_codePtr)
        m_codePtr =
            m_builder.CreateGlobalStringPtr({reinterpret_cast<char const*>(m_codeBegin),
                                                static_cast<size_t>(m_codeEnd - m_codeBegin)},
                "code");
    return m_codePtr;
}
