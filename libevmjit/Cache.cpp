...
std::string getVersionedCacheDir()
{
    ...
    llvm::sys::path::user_cache_directory(
        path, "ethereum", "evmjit", std::to_string(c_internalABIVersion));
    return path.str();
}
...
void Cache::clear()
{
    ...
    std::error_code err;
    for (auto it = llvm::sys::fs::directory_iterator{cachePath, err}; it != decltype(it){};
         it.increment(err))
        llvm::sys::fs::remove(it->path());
}

void Cache::preload(llvm::ExecutionEngine& _ee,
    std::unordered_map<std::string, uint64_t>& _funcCache, llvm::LLVMContext& _llvmContext)
{
    Guard g{x_cacheMutex};
...
std::unique_ptr<llvm::Module> Cache::getObject(
    std::string const& id, llvm::LLVMContext& _llvmContext)
{
    Guard g{x_cacheMutex};
