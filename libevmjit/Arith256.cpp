...
namespace jit
{
Arith256::Arith256(IRBuilder& _builder) : CompilerHelper(_builder) {}
...
void Arith256::debug(llvm::Value* _value, char _c, llvm::Module& _module, IRBuilder& _builder)
{
    ...
    if (!func)
        func = llvm::Function::Create(
            llvm::FunctionType::get(Type::Void, {Type::Word, _builder.getInt8Ty()}, false),
            llvm::Function::ExternalLinkage, funcName, &_module);

    _builder.CreateCall(
        func, {_builder.CreateZExtOrTrunc(_value, Type::Word), _builder.getInt8(_c)});
}
...
{
    // Based of "Improved shift divisor algorithm" from "Software Integer Division" by Microsoft
    // Research The following algorithm also handles divisor of value 0 returning 0 for both
    // quotient and remainder

    auto retType = llvm::VectorType::get(_type, 2);
    auto func = llvm::Function::Create(llvm::FunctionType::get(retType, {_type, _type}, false),
        llvm::Function::PrivateLinkage, _funcName, &_module);
    ...
    auto ret =
        builder.CreateInsertElement(llvm::UndefValue::get(retType), qRet, uint64_t(0), "ret0");
...
llvm::Function* Arith256::getUDiv256Func(llvm::Module& _module)
{
    ...
    auto func =
        llvm::Function::Create(llvm::FunctionType::get(Type::Word, {Type::Word, Type::Word}, false),
            llvm::Function::PrivateLinkage, funcName, &_module);
    func->setDoesNotThrow();
    func->setDoesNotAccessMemory();
...
{
    auto udivremFunc = _type == Type::Word ? Arith256::getUDivRem256Func(_module) :
                                             Arith256::getUDivRem512Func(_module);

    auto func = llvm::Function::Create(llvm::FunctionType::get(_type, {_type, _type}, false),
        llvm::Function::PrivateLinkage, _funcName, &_module);
    func->setDoesNotThrow();
    func->setDoesNotAccessMemory();
...
llvm::Function* Arith256::getSDivRem256Func(llvm::Module& _module)
{
    ...
    auto func =
        llvm::Function::Create(llvm::FunctionType::get(retType, {Type::Word, Type::Word}, false),
            llvm::Function::PrivateLinkage, funcName, &_module);
    func->setDoesNotThrow();
    func->setDoesNotAccessMemory();
...
llvm::Function* Arith256::getExpFunc()
{
    if (!m_exp)
    {
        ...
        m_exp = llvm::Function::Create(llvm::FunctionType::get(Type::Word, argTypes, false),
            llvm::Function::PrivateLinkage, "exp", getModule());
        ...
        auto eOdd = m_builder.CreateICmpNE(
            m_builder.CreateAnd(e, Constant::get(1)), Constant::get(0), "e.isodd");
...
EXPORT void debug(uint64_t a, uint64_t b, uint64_t c, uint64_t d, char z)
{
    DLOG(JIT) << "DEBUG " << std::dec << z << ": "  //<< d << c << b << a
              << " [" << std::hex << std::setfill('0') << std::setw(16) << d << std::setw(16) << c
              << std::setw(16) << b << std::setw(16) << a << "]\n";
}
}
