...
private:
    ...
    /// Local stack items that has not been pushed to global stack. First item is just above global
    /// stack.
    std::vector<llvm::Value*> m_local;

    llvm::CallInst* m_sp = nullptr;  ///< Call to stack.prepare function which returns stack pointer
                                     ///< for current basic block.

    ssize_t m_globalPops = 0;  ///< Number of items poped from global stack. In other words: global
                               ///< - local stack overlap.
...
{
public:
    explicit BasicBlock(instr_idx _firstInstrIdx, code_iterator _begin, code_iterator _end,
