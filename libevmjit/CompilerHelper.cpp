...
namespace jit
{
CompilerHelper::CompilerHelper(IRBuilder& _builder) : m_builder(_builder) {}
...
llvm::Function* CompilerHelper::getMainFunction()
{
    ...
    assert(mainFunc);
    if (mainFunc == &mainFunc->getParent()->getFunctionList().front())  // Main function is the
                                                                        // first one in module
        return mainFunc;
    return nullptr;
}
...
RuntimeHelper::RuntimeHelper(RuntimeManager& _runtimeManager)
  : CompilerHelper(_runtimeManager.m_builder), m_runtimeManager(_runtimeManager)
{}
