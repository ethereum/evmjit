...
llvm::Value* LazyFunction::call(
    IRBuilder& _builder, std::initializer_list<llvm::Value*> const& _args, llvm::Twine const& _name)
{
    if (!m_func)
        m_func = m_creator();
...
llvm::Function* Array::createArrayPushFunc()
{
    ...
    auto func = llvm::Function::Create(llvm::FunctionType::get(Type::Void, argTypes, false),
        llvm::Function::PrivateLinkage, "array.push", getModule());
...
llvm::Function* Array::createArraySetFunc()
{
    ...
    auto func = llvm::Function::Create(llvm::FunctionType::get(Type::Void, argTypes, false),
        llvm::Function::PrivateLinkage, "array.set", getModule());
    func->setDoesNotThrow();
    func->addAttribute(1, llvm::Attribute::NoCapture);
...
llvm::Function* Array::createArrayGetFunc()
{
    ...
    auto func = llvm::Function::Create(llvm::FunctionType::get(Type::Word, argTypes, false),
        llvm::Function::PrivateLinkage, "array.get", getModule());
    func->setDoesNotThrow();
    func->addAttribute(1, llvm::Attribute::NoCapture);
...
{
    llvm::Type* argTypes[] = {m_array->getType(), Type::Size};
    auto func = llvm::Function::Create(llvm::FunctionType::get(Type::WordPtr, argTypes, false),
        llvm::Function::PrivateLinkage, "array.getPtr", getModule());
    func->setDoesNotThrow();
    func->addAttribute(1, llvm::Attribute::NoCapture);
...
{
    auto func =
        llvm::Function::Create(llvm::FunctionType::get(Type::Void, m_array->getType(), false),
            llvm::Function::PrivateLinkage, "array.free", getModule());
    func->setDoesNotThrow();
    func->addAttribute(1, llvm::Attribute::NoCapture);

    auto freeFunc =
        llvm::Function::Create(llvm::FunctionType::get(Type::Void, Type::BytePtr, false),
...
llvm::Function* Array::getReallocFunc()
{
    ...
    auto reallocFunc =
        llvm::Function::Create(llvm::FunctionType::get(Type::BytePtr, reallocArgTypes, false),
            llvm::Function::ExternalLinkage, "realloc", getModule());
...
llvm::Function* Array::createExtendFunc()
{
    llvm::Type* argTypes[] = {m_array->getType(), Type::Size};
    auto func = llvm::Function::Create(llvm::FunctionType::get(Type::Void, argTypes, false),
        llvm::Function::PrivateLinkage, "array.extend", getModule());
    func->setDoesNotThrow();
    func->addAttribute(1, llvm::Attribute::NoCapture);
    ...
    InsertPointGuard guard{m_builder};
    m_builder.SetInsertPoint(llvm::BasicBlock::Create(m_builder.getContext(), {}, func));
    auto dataPtr = m_builder.CreateBitCast(
        arrayPtr, Type::BytePtr->getPointerTo(), "dataPtr");  // TODO: Use byte* in Array
    auto sizePtr = m_builder.CreateStructGEP(getType(), arrayPtr, 1, "sizePtr");
    auto capPtr = m_builder.CreateStructGEP(getType(), arrayPtr, 2, "capPtr");
    auto data = m_builder.CreateLoad(dataPtr, "data");
    auto size = m_builder.CreateLoad(sizePtr, "size");
    auto extSize = m_builder.CreateNUWSub(newSize, size, "extSize");
    auto newData = m_reallocFunc.call(
        m_builder, {data, newSize}, "newData");  // TODO: Check realloc result for null
...
Array::Array(IRBuilder& _builder, char const* _name) : CompilerHelper(_builder)
{
    m_array = m_builder.CreateAlloca(getType(), nullptr, _name);
    m_builder.CreateStore(llvm::ConstantAggregateZero::get(getType()), m_array);
}

Array::Array(IRBuilder& _builder, llvm::Value* _array) : CompilerHelper(_builder), m_array(_array)
{
    m_builder.CreateStore(llvm::ConstantAggregateZero::get(getType()), m_array);
}
